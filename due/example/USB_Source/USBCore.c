...
#ifndef USB_PRODUCT
#define USB_PRODUCT "Xeb360-USBDevice"
#endif

const uint8_t STRING_PRODUCT[] = USB_PRODUCT;

#ifndef USB_MANUFACTURER
#define USB_MANUFACTURER "Xebrium Inc"
#endif
...
//==================================================================
//==================================================================
struct _PluggableUSB_ *PluggableUSB;
...
void LockEP(uint32_t ep __attribute__ ((unused))) {
	_LockEP.flags = cpu_irq_save();
}

void UnLockEP()
{
	cpu_irq_restore(_LockEP.flags);
}

/**
 * \brief Delay number of tick Systicks (happens every 1 ms).
 */
__INLINE static void delay(uint32_t dw_dly_ticks)
{
	uint32_t dw_cur_ticks;

	dw_cur_ticks = GetTickCount();
...
uint32_t USBD_Available(uint32_t ep)
{
	uint32_t bytes;
	LockEP(ep);
	bytes = UDD_FifoByteCount(ep & 0xF);
	UnLockEP();
	return bytes;
}
...
uint32_t USBD_Recv_byte(uint32_t ep)
{
	uint8_t c;
...
int USBD_SendControl(uint8_t flags __attribute__ ((unused)), const void* d, uint32_t len)
{
	...
	printf_lite("=> USBD_SendControl TOTAL len=%u\r\n", len);
	...
	if (_cmark < _cend)
	{
		while (len > 0)
		{
			...
			printf_lite("=> USBD_SendControl sent=%u\r\n", sent);
			pos += sent;
			len -= sent;
		}
	}
...
bool USBD_ClassInterfaceRequest(USBSetup setup)
{
	...
	printf_lite("=> USBD_ClassInterfaceRequest\r\n");
...
uint8_t USBD_SendInterfaces(void)
{
	...
	printf_lite("=> USBD_SendInterfaces, interfaces=%d\r\n", interfaces);
	return interfaces;
}
...
static bool USBD_SendConfiguration(int maxlen)
{
	...
	USBD_InitControl(0);
	//printf_lite("=> USBD_SendConfiguration _cmark1=%d\r\n", _cmark);
	uint8_t interfaces = USBD_SendInterfaces();
	//printf_lite("=> USBD_SendConfiguration _cmark2=%d\r\n", _cmark);
	//printf_lite("=> USBD_SendConfiguration sizeof=%d\r\n", sizeof(ConfigDescriptor));

_Pragma("pack(1)")
	ConfigDescriptor config = D_CONFIG(_cmark + sizeof(ConfigDescriptor),interfaces);
_Pragma("pack()")
	//printf_lite("=> USBD_SendConfiguration clen=%d\r\n", config.clen);

	//printf_lite("=> USBD_SendConfiguration maxlen=%d\r\n", maxlen);
...
static bool USBD_SendDescriptor(USBSetup setup)
{
	...
	{
		printf_lite("=> USBD_SendDescriptor : USB_CONFIGURATION_DESCRIPTOR_TYPE length=%d\r\n", setup.wLength);
	...
	{
		printf_lite("=> USBD_SendDescriptor : USB_DEVICE_DESCRIPTOR_TYPE\r\n");
		if (setup.wLength == 8)
		{
			_cdcComposite = 1;
		}
	...
	{
		printf_lite("=> USBD_SendDescriptor : USB_STRING_DESCRIPTOR_TYPE\r\n");
		if (setup.wValueL == 0) {
	...
	printf_lite("=> USBD_SendDescriptor : desc_addr=%p desc_length=%d\r\n", desc_addr, desc_length);
...
//    if( iii++ > 1500 ) while(1); // jcb
    // End of bus reset
	printf_lite("======== USB_ISR Executed ========\r\n\0");

    if (Is_udd_reset())
    {
		printf_lite(">>> End of Reset\r\n");
	...
	if (Is_udd_endpoint_interrupt(0) )
	{
		...
		{
			printf_lite(">>> EP0 Int: IN Request\r\n");
			UDD_WaitIN();
		}
		else
		{
			printf_lite(">>> EP0 Int: OUT Request\r\n");
			UDD_ClearIN();
		}
		...
		if (REQUEST_STANDARD == (requestType & REQUEST_TYPE))
		{
			...
			if (GET_STATUS == r)
			{
                if( setup.bmRequestType == 0 )  // device
                {
                    // Send the device status
     				printf_lite(">>> EP0 Int: GET_STATUS\r\n");
			...
			{
				printf_lite(">>> EP0 Int: SET_ADDRESS\r\n");
				UDD_WaitIN();
				UDD_SetAddress(setup.wValueL);
			}
			else if (GET_DESCRIPTOR == r)
			{
				printf_lite(">>> EP0 Int: GET_DESCRIPTOR\r\n");
				ok = USBD_SendDescriptor(setup);
			}
			else if (SET_DESCRIPTOR == r)
			{
				printf_lite(">>> EP0 Int: SET_DESCRIPTOR\r\n");
				ok = false;
			}
			else if (GET_CONFIGURATION == r)
			{
				printf_lite(">>> EP0 Int: GET_CONFIGURATION\r\n");
			...
			else if (SET_CONFIGURATION == r)
			{
				...
				{
					printf_lite(">>> EP0 Int: SET_CONFIGURATION REQUEST_DEVICE %d\r\n", setup.wValueL);
				...
				{
					printf_lite(">>> EP0 Int: SET_CONFIGURATION failed!\r\n");
					ok = false;
				}
			}
			else if (GET_INTERFACE == r)
			{
				printf_lite(">>> EP0 Int: GET_INTERFACE\r\n");
				UDD_Send8(EP0, _usbSetInterface);
			}
			else if (SET_INTERFACE == r)
			{
                _usbSetInterface = setup.wValueL;
				printf_lite(">>> EP0 Int: SET_INTERFACE\r\n");
			}
		}
		else
		{
			printf_lite(">>> EP0 Int: ClassInterfaceRequest\r\n");
		...
		if (ok)
		{
			printf_lite(">>> EP0 Int: Send packet\r\n");
			UDD_ClearIN();
		}
		else
		{
			printf_lite(">>> EP0 Int: Stall\r\n");
			UDD_Stall();
		}
	}
}
...
void USBDevice_Init(void)
{
	...
	PluggableUSB = PluggableUSB_Init();
}
