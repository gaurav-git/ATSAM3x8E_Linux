...
INCLUDES += -I$(ROOT)/sam/libsam/include
INCLUDES += -I$(ROOT)/sam/CMSIS/CMSIS/Include
INCLUDES += -I$(ROOT)/sam/CMSIS/Device/ATMEL
INCLUDES += -I$(ROOT)/sam/CMSIS/Device/ATMEL/sam3xa/include/
INCLUDES += -I$(ROOT)/sam/CMSIS/Device/ATMEL/sam3xa/include/component
INCLUDES += -I$(ROOT)/example/USB_Source
...
USB_C_SRC= $(wildcard $(PROJECT_DIR)/USB_Source/*.c)
USB_C_OBJ+=$(patsubst %.c, %.o, $(notdir $(USB_C_SRC)))
...
# .c -> .o
$(addprefix $(OBJ_DIR)/,$(USB_C_OBJ)): $(OBJ_DIR)/%.o: $(USB_C_SRC)
	"$(CC)" -c $(CFLAGS) $< -o $@
...
# .o -> .a
$(OBJ_DIR)/$(OUTPUT_NAME).a: $(addprefix $(OBJ_DIR)/, $(C_OBJ)) $(addprefix $(OBJ_DIR)/, $(USB_C_OBJ)) $(addprefix $(OBJ_DIR)/, $(CPP_OBJ)) $(addprefix $(OBJ_DIR)/, $(A_OBJ))
	"$(AR)" $(ARFLAGS) $@ $^
	"$(NM)" $@ > $@.txt
